...
def test_searching_existing_resources(session):
    ...
    assert isinstance(searchResourcesResults1, tuple), "return type of searchResources(query) should be a tuple"
    assert (
        searchResourcesResults1[0] == "ff"
    ), "0th element of returned tuple should be the query 'ff'"
    assert(searchResourcesResults1[1] == [tResource1]), "should return True because only 1 resource matches up with a query of 'ff'"
